...
fn validate_selections(
    ...
    if let Some(session) = web::get_sesssion_from_cookies(&mut cookies, &session_store) {
        ...
        for session in &update.updated_sessions {
            ...
            if session_ids.contains(&&session.id) {
                warn!("Session ID does not belong. Session ids: {:?}, session id: {}", session_ids, session.id);
                return Err(status::Custom(Status::InternalServerError, "Session does not belong to event!".to_string()))
            }
        }
